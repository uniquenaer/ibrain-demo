...
const rawCsv = fs.readFileSync(path.join(__dirname, '3fixedTopic.csv'));
...
parse(rawCsv, { header: true, skip_empty_lines: true }, function (err, output) {
    ...
    fs.writeFile(path.join(__dirname, '../src/data/packList_three.js'), `const packList_three = ${JSON.stringify(packArr)}; export default packList_three`, 'utf8');
    fs.writeFile(path.join(__dirname, '../src/data/cardList_three.js'), `const cardList_three = ${JSON.stringify(cardArr)}; export default cardList_three`, 'utf8');
});
